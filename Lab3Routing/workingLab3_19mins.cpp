...
int main() {
    parseGMP("testcase1.gmp");
    parseGCL("testcase1.gcl");
    parseCST("testcase1.cst");

    int bumpCount = chips[0].bumps.size();
    vector<int> bumpOrder(bumpCount);
    iota(bumpOrder.begin(), bumpOrder.end(), 0); // 0,1,2,...

    auto startTime = chrono::steady_clock::now();
    const auto timeLimit = chrono::minutes(19);
    ...
    do {
        ...
        permutationCounter++;
    } while (next_permutation(bumpOrder.begin(), bumpOrder.end()));

    auto endTime = chrono::steady_clock::now();
    auto duration = chrono::duration_cast<chrono::seconds>(endTime - startTime);

    cout << "Best total cost: " << bestTotalCost << endl;
    cout << "Total permutations tried: " << permutationCounter << endl;
    cout << "Execution time: " << duration.count() << " seconds" << endl;

    printSolutionFile(bestSolution, "testcase1.lg");
