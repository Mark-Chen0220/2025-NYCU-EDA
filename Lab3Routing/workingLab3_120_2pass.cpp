...
void costFunction(bool layerSwap, Gcell &cell, bool useCongestion){
    double cost = 0;
    ...
    if (useCongestion) {
        if (cell.topEdgeNet + 1 - cell.topEdgeCapacity > 0)
            cost += (cell.topEdgeNet - cell.topEdgeCapacity) * overflowCost * overflowWeight;
        if (cell.bottomEdgeNet + 1 - cell.bottomEdgeCapacity > 0)
            cost += (cell.bottomEdgeNet - cell.bottomEdgeCapacity) * overflowCost * overflowWeight;
        if (cell.leftEdgeNet + 1 - cell.leftEdgeCapacity > 0)
            cost += (cell.leftEdgeNet - cell.leftEdgeCapacity) * overflowCost * overflowWeight;
        if (cell.rightEdgeNet + 1 - cell.rightEdgeCapacity > 0)
            cost += (cell.rightEdgeNet - cell.rightEdgeCapacity) * overflowCost * overflowWeight;
    }
...
vector<vector<pair<int,int>>> performRouting(const vector<int>& bumpOrder, double& totalCostOut, bool useCongestion) {
    ...
    for (int i : bumpOrder) {
        ...
        for (int x = 0; x < gridCols; ++x) {
            for (int y = 0; y < gridRows; ++y) {
                ra.GcellGrid[x][y].cost = alpha * (
                    abs(targetX - x) * grid.Xlength + 
                    abs(targetY - y) * grid.Yheight
                );
            }
        }
        ...
        CellNode startingNode = { ra.GcellGrid[startX][startY].cost, &ra.GcellGrid[startX][startY], 1 };
        ...
        while (!openHeap.empty()) {
            ...
            for (auto &d : dirs) {
                ...
                int nextMetal = (cm == 1 ^ switchMetal) ? 2 : 1;
                ra.GcellGrid[nx][ny].reachedUsingMetal = nextMetal;
                costFunction(switchMetal, ra.GcellGrid[nx][ny], useCongestion);
                ...
                if (tentativeG < gScore[nx][ny]) {
                    ...
                    openHeap.push({fScore, &ra.GcellGrid[nx][ny], nextMetal == 1 ? 1 : 2});
                }
            }
        }
...
int main() {
    parseGMP("testcase3.gmp");
    parseGCL("testcase3.gcl");
    parseCST("testcase3.cst");
    ...
    while (tries < maxTries) {
        ...
        // --- First pass: route ignoring congestion ---
        double dummyCost = 0.0;
        performRouting(bumpOrder, dummyCost, false); // edge usage gets filled

        // --- Second pass: now consider congestion ---
        // Reset edge usage again (we will rebuild it based on second routing)
        for (int y = 0; y < gridRows; y++){
            for (int x = 0; x < gridCols; x++){
        ...
        vector<vector<pair<int,int>>> solution = performRouting(bumpOrder, currentCost, true); // second pass with penalties
    ...
    printSolutionFile(bestSolution, "testcase3.lg");
    return 0;
}