...
void parseGCL(string filename){
    ...
    for (int i = 0; i < gridRows; i++){
        for (int j = 0; j < gridCols; j++){
            ...
            ra.GcellGrid[i][j].topEdgeCapacity = values[1];
            ra.GcellGrid[i][j].bottomEdgeCapacity = values[1];
            ra.GcellGrid[i][j].leftEdgeCapacity = values[0];
            ra.GcellGrid[i][j].rightEdgeCapacity = values[0];
        }
    }
    file.close();
}
...
vector<vector<pair<int,int>>> performRouting() {
    ...
    for (size_t i = 0; i < chips[0].bumps.size(); ++i) {
        ...
        vector<vector<float>> gScore(gridCols, vector<float>(gridRows, INF));
        ...
        while (!openHeap.empty()) {
            ...
            for (auto &d : dirs) {
                ...
                float tentativeG = gScore[cx][cy] + ra.GcellGrid[nx][ny].cost;
                if (tentativeG < gScore[nx][ny]) {
                    cameFrom[nx][ny] = {cx, cy};
                    gScore[nx][ny] = tentativeG;
                    float fScore = tentativeG + ra.GcellGrid[nx][ny].cost;
